...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for n in range(N-1):
        ...
        for measurement in measurements:
            for i in range(2):
                landmark = int(measurement[0])
                omega.value[2*n+i][2*n+i] += 1/measurement_noise 
                omega.value[2*n+i][2*N+2*landmark+i] -= 1 /measurement_noise 
                omega.value[2*N+2*landmark+i][2*n+i] -= 1 /measurement_noise 
                omega.value[2*N+2*landmark+i][2*N+2*landmark+i] += 1/measurement_noise 
                xi.value[2*n+i][0] -= measurement[i+1]/measurement_noise 
                xi.value[2*N+2*landmark+i][0] += measurement[i+1]/measurement_noise 
        
        for i in range(2):
            omega.value[2*n+i][2*n+i] += 1/motion_noise
            omega.value[2*(n+1)+i][2*n+i] -= 1/motion_noise
            omega.value[2*n+i][2*(n+1)+i] -= 1/motion_noise
            omega.value[2*(n+1)+i][2*(n+1)+i] += 1/motion_noise
            xi.value[2*n+i][0] -= motions[i]/motion_noise
            xi.value[2*(n+1)+i][0] += motions[i]/motion_noise
    mu = omega.inverse()*xi
